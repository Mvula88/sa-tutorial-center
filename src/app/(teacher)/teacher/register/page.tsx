...
import { BookOpen, Mail, Lock, Loader2, AlertCircle, Eye, EyeOff, CheckCircle, User } from 'lucide-react'
...
function TeacherRegisterContent() {
  ...
  async function validateToken() {
    try {
      ...
      if (result.success && result.entityType === 'teacher') {
        setTeacherId(result.entityId)
        setTeacherName(result.entityName || '')
        if (result.entityEmail) {
          setTeacherEmail(result.entityEmail)
  ...
  async function handleRegister(e: React.FormEvent) {
    ...
    try {
      ...
      setTimeout(() => {
        router.push('/portal/login?type=teacher')
      }, 3000)
    } catch (err) {
        ...
        <div className="w-full max-w-md">
          <div className="bg-white rounded-2xl shadow-xl p-8 text-center">
            ...
            <p className="text-gray-600 mb-4">
              Your account has been created. You can now log in to your teacher portal.
            </p>
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8">
            <div className="text-center">
              ...
              <p className="text-gray-600 mb-6">
                To register as a teacher, you need a registration link from your tutorial center administrator.
              </p>
              <p className="text-sm text-gray-500 mb-6">
                Please contact your center admin to get your personalized registration link.
              </p>
              <Link
                href="/portal/login?type=teacher"
                className="inline-block w-full py-2.5 bg-emerald-600 text-white font-medium rounded-lg hover:bg-emerald-700 transition-colors text-center"
              >
                Go to Login
              </Link>
            </div>
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
          ...
          <div className="flex items-center gap-3 p-4 bg-emerald-50 rounded-lg mb-6">
            ...
            <div>
              <p className="font-medium text-emerald-800">{teacherName}</p>
              <p className="text-sm text-emerald-600">Teacher Account</p>
            </div>
          </div>
            ...
            <div>
              ...
              {teacherEmail && email !== teacherEmail && (
                <p className="text-xs text-amber-600 mt-1">
                  Note: This is different from your registered email ({teacherEmail})
                </p>
              )}
            </div>
          ...
          <div className="mt-6 pt-6 border-t border-gray-200">
            <p className="text-center text-sm text-gray-600">
              ...
              <Link href="/portal/login?type=teacher" className="text-emerald-600 hover:text-emerald-700 font-medium">
                Sign in here
              </Link>
            </p>
          </div>
