  ...
  async function fetchClient() {
    ...
    try {
      ...
      setFormData({
        ...
        city: client.city || '',
        has_website: client.has_website,
        has_school_management: client.has_school_management,
        website_domain: client.website_domain || '',
        website_url: client.website_url || '',
        domain_expiry_date: client.domain_expiry_date || '',
        hosting_expiry_date: client.hosting_expiry_date || '',
        contract_start_date: client.contract_start_date,
        setup_fee: client.setup_fee,
        setup_fee_paid: client.setup_fee_paid,
        setup_fee_paid_date: client.setup_fee_paid_date || '',
        monthly_sms_fee: client.monthly_sms_fee,
        annual_website_fee: client.annual_website_fee,
        notes: client.notes || '',
      })
    } catch (error) {
    ...
    try {
      const { error } = await (supabase
        ...
        .update({
          ...
          domain_expiry_date: formData.has_website && formData.domain_expiry_date ? formData.domain_expiry_date : null,
          hosting_expiry_date: formData.has_website && formData.hosting_expiry_date ? formData.hosting_expiry_date : null,
          contract_start_date: formData.contract_start_date,
          setup_fee: formData.setup_fee,
          setup_fee_paid: formData.setup_fee_paid,
          setup_fee_paid_date: formData.setup_fee_paid && formData.setup_fee_paid_date ? formData.setup_fee_paid_date : null,
  ...
  return (
    <div className="p-4 md:p-8 max-w-4xl mx-auto">
      ...
      <div className="mb-6 md:mb-8">
        ...
        <h1 className="text-xl md:text-2xl font-bold text-gray-900">Edit Client</h1>
        <p className="text-gray-500 mt-1 text-sm md:text-base">Update client information</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="pl-9 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <Input
                    ...
                    placeholder="e.g., https://absolutetutorial.com"
                  />
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <Input
                    ...
                    onChange={(e) => setFormData({ ...formData, domain_expiry_date: e.target.value })}
                  />
                  <Input
                    label="Hosting Expiry Date"
                    type="date"
                    value={formData.hosting_expiry_date}
                    onChange={(e) => setFormData({ ...formData, hosting_expiry_date: e.target.value })}
                  />
                </div>
                <Input
                  label="Annual Website Fee (N$)"
                  type="number"
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h2 className="text-lg font-semibold text-gray-900">Contract & Fees</h2>
          </div>
          ...
          <div className="mt-4 p-4 bg-gray-50 rounded-lg">
            ...
            {formData.setup_fee_paid && (
              <div className="mt-3 pl-8">
                <Input
                  label="Setup Fee Paid Date"
                  type="date"
                  value={formData.setup_fee_paid_date}
                  onChange={(e) => setFormData({ ...formData, setup_fee_paid_date: e.target.value })}
                />
              </div>
            )}
          </div>
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="border-t border-gray-200 pt-2 mt-2">
                <div className="flex justify-between font-medium">
                  <span className="text-gray-900">Annual Total</span>
